...
contract PiTreasury is Ownable {
    ...
    address public sXRX;
    uint256 public sXRXQueue; // Delays change to sXRX address
    ...
    /**
        @notice allow approved address to deposit an asset for XRX
        @param _amount uint
    ...
    function deposit(
        ...
        // mint XRX needed and store amount of rewards for distribution
        // s = 100000000000 - 99909426854 = 0.090573146
        send_ = value.sub(_profit);
        IERC20Mintable(XRX).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn XRX for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IXRXERC20(XRX).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sXRX).balanceOf(msg.sender); // Can only borrow against sXRX held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with XRX
        @param _amount uint
     */
    function repayDebtWithXRX(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IXRXERC20(XRX).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, XRX, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IERC20Mintable(XRX).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(XRX).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns XRX valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match XRX decimals
            value_ = _amount.mul(10**IERC20(XRX).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.sXRX) {
            // 9
            sXRXQueue = block.number.add(blocksNeededForQueue);
        } else return false;
