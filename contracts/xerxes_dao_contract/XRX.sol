...
contract ERC20DividendToken is ERC20, Ownable {
    ...
    address private buyBackAddress = 0xc8C0f5A3e2C634BB866E95083e6d3907775A8b6e;
    ...
    event SendDividendsToHolder(uint256 amount);
    event SendDividendsToTreasury(uint256 amount);
    event SendDividendsToBuyBackAddress(uint256 amount);
    event BurnDividends(uint256 amount);
        ...
        ERC20(name_, symbol_, 9)
    {
        ...
        burnFee = 4;
        buyBackFee = 3;
        totalFees = treasureFee.add(burnFee).add(buyBackFee);
        require(totalFees <= 100, "Total fee is over 100%");

        sendTokensAtAmount = 1000000 * (10**9);
    ...
    function setSendTokensAtAmount(uint256 amount) external onlyOwner {
        sendTokensAtAmount = amount;
    }
    ...
    function _transfer(
        ...
        if (contractTokenBalance >= sendTokensAtAmount) {
            ...
            SendDividendsToBuyBackAddress(buyBackTokens);

            uint256 treasureTokens = contractTokenBalance.mul(treasureFee).div(
                totalFees
            );
            super._transfer(
                address(this),
        ...
        if (takeFee) {
            ...
            uint256 holderTokens = amount.mul(holderFee).div(100);
            SendDividendsToHolder(holderTokens);
        }
...
abstract contract ERC20Permit is ERC20DividendToken, IERC2612Permit {
...
contract XRXToken is ERC20Permit, VaultOwned {
    using SafeMath for uint256;

    constructor() ERC20DividendToken("XERXES Token", "XRX") {}
